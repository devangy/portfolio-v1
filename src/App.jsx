...
const SOCIALS = [
    { name: "GitHub", icon: Github, url: "https://github.com/devangy" },
    {
        name: "LinkedIn",
        icon: Linkedin,
        url: "https://www.linkedin.com/in/devangy",
    },
    ...
    return (
        <div className="relative min-h-screen bg-black text-white font-jetbrains overflow-x-clip selection:bg-cyan-400/30">
            ...
            <button
                ...
                className="hidden sm:grid fixed top-4 left-4 sm:top-5 sm:left-5 z-50 w-8 h-8 sm:w-11 sm:h-11 rounded-full place-items-center bg-white/[0.04] backdrop-blur-md border border-white/10 hover:border-cyan-300/40 hover:bg-white/[0.08] transition-all"
            ...
            <main
                className={`relative z-10 mx-auto px-4 sm:px-5 pt-24 sm:pt-28 pb-24 ${tab === "portfolio" ? "max-w-5xl" : "max-w-3xl"}`}
            >
            ...
            <div className="flex-1 min-w-0 w-full text-center sm:text-left">
                ...
                <div className="mt-5 pt-4 border-t border-white/[0.06] flex items-center gap-5 text-[12px] text-white/45 font-jetbrains">
                    <span className="flex items-center gap-1.5">
                        <MapPin size={13} className="text-emerald-300/70" />{" "}
                        <span className="text-amber-200/80">Mumbai</span>,{" "}
    ...
    <Card className="flex overflow-hidden">
        ...
        <div className="flex-1 flex items-center gap-3 px-4 py-3 min-w-0">
            <div className="flex-1 min-w-0">
                <div className="text-[14px] text-white font-jetbrains leading-tight">
                    Devang Yadav
                </div>
                <div className="mt-0.5 text-[11px] text-white/45 font-jetbrains">
                    @devangy · fullstack
                </div>
        ...
        <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
            {PROJECTS.slice(0, 2).map((p, i) => {
                ...
                return (
                    <Card
                        key={p.name}
                        className="p-4 group hover:border-white/20 transition-colors"
                    >
                        <div className="flex items-center gap-3">
                            <div
                                className={`w-11 h-11 rounded-xl bg-gradient-to-br ${a.from} ${a.to} border border-white/[0.08] grid place-items-center font-spacemono text-[13px] ${a.text}`}
                            >
                                {p.name.slice(0, 2).toUpperCase()}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="text-[14px] text-white font-jetbrains truncate">
                                    {p.name}
                                </div>
                                <div className="text-[11px] text-white/45 font-jetbrains">
                                    {p.tag}
                                </div>
                            </div>
                            <a
...
const SectionHead = ({ Icon, title, color = "emerald" }) => {
    ...
    return (
        <div className="flex items-center gap-2.5 mb-4 text-white/85">
            ...
            <h3 className="text-[16px] font-spacemono tracking-wider">
                {title}
            </h3>
    ...
    <Card className="p-6 sm:p-8">
        ...
        <div className="space-y-4">
            <ExpTile
                ...
                sub="March 2026 – April 2026 · Remote"
...
const ExpTile = ({
    ...
    >
        <div
            className={`absolute left-0 top-0 bottom-0 w-[3px] bg-gradient-to-b ${ACCENT_BAR[accent]}`}
        />
        <div className="flex items-start justify-between gap-4">
            <div>
                <div className="text-[19px] sm:text-[22px] text-white font-jetbrains leading-snug">
                    {title}
                </div>
                {role && (
                    <div
                        className={`mt-0.5 text-[15px] sm:text-[16px] font-jetbrains ${PREVIEW_TEXT[accent]}`}
                    >
                        {role}
                    </div>
                )}
                <div className="mt-1 text-[13px] text-white/40 font-jetbrains">
                    {sub}
                </div>
            </div>
            <ExternalLink
            ...
            <>
                ...
                <ul className="mt-4 space-y-3.5">
                    {bullets.map((b, i) => (
                        <li
                            key={i}
                            className="flex gap-3 text-[14px] sm:text-[15px] text-white/72 leading-relaxed"
                        >
                            <span
                                className={`mt-[8px] shrink-0 w-2 h-2 rounded-full ${ACCENT_DOT[accent]} opacity-70 shadow-[0_0_6px_currentColor]`}
                            />
                            {b}
                        </li>
                    ))}
        ...
        <div className="mt-5">
            <span
                className={`inline-block px-2.5 py-1 text-[11px] uppercase tracking-wider rounded-md border font-jetbrains ${KIND_COLORS[accent]}`}
            >
                {kind}
            </span>
        </div>
                ...
                return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2.5">
                            <div
                                className={`relative shrink-0 w-10 h-10 sm:w-14 sm:h-14 rounded-lg overflow-hidden border border-white/[0.08] bg-gradient-to-br ${PREVIEW_GRADIENTS[accent]} grid place-items-center`}
                            >
                                ...
                                <span
                                    className={`relative text-lg sm:text-2xl ${PREVIEW_TEXT[accent]} drop-shadow-[0_0_12px_currentColor]`}
                                >
                                    {glyph}
                                </span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="text-[15px] sm:text-[19px] text-white font-spacemono leading-tight truncate">
                                    {p.name}
                                </div>
                                <div
                                    className={`mt-0.5 text-[10px] sm:text-[12px] uppercase tracking-[0.15em] font-jetbrains ${accent === "cyan" ? "text-cyan-300" : accent === "emerald" ? "text-emerald-300" : "text-sky-300"}`}
                                >
                                    {p.tag}
                                </div>
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                                ...
                                <a
                                    ...
                                    style={{
                                        textShadow: `0 0 8px ${chipColor.shadow}`,
                                    }}
                                >
                        ...
                        {/* Description */}
                        <p className="mt-2 text-[11.5px] sm:text-[15px] text-white/55 leading-snug sm:leading-relaxed">
                            {p.desc}
                        </p>
                            ...
                            <>
                                ...
                                <ul className="mt-2 space-y-1.5 sm:space-y-2.5">
                                    {p.bullets.map((b, bi) => (
                                        <li
                                            key={bi}
                                            className="flex gap-1.5 text-[11px] sm:text-[16px] text-white/60 leading-snug sm:leading-relaxed"
                                        >
                                            <span
                                                className={`mt-[5px] shrink-0 w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full ${ACCENT_DOT[accent]} opacity-65 shadow-[0_0_5px_currentColor]`}
                                            />
                                            {b}
                                        </li>
                                    ))}
                        ...
                        <div className="mt-2 sm:mt-3 flex flex-wrap gap-1 sm:gap-1.5">
                            {p.stack.map((s) => (
                                <span
                                    key={s}
                                    className={`flex items-center gap-1 text-[9.5px] sm:text-[13px] px-2 sm:px-3 py-0.5 sm:py-1 rounded-full border font-jetbrains ${chipColor.text} ${chipColor.border} ${chipColor.bg}`}
                                    style={{
                                        textShadow: `0 0 8px ${chipColor.shadow}`,
                                    }}
                                >
...
const ContactCard = () => (
    <Card className="p-6 sm:p-8">
        ...
        <p className="text-[13.5px] text-white/65 leading-relaxed">
            Open to fullstack roles, freelance projects and collabs on weird
            ideas. Reply within a day usually — sometimes faster if I&apos;m
            awake.
        </p>
...
const SOCIAL_HOVERS = {
    ...
    LinkedIn:
        "hover:border-cyan-400/40 hover:bg-cyan-500/[0.06] hover:text-cyan-200 hover:shadow-[0_0_18px_rgba(34,211,238,0.3)]",
