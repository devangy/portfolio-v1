...
export const AmbientPlayer = ({ muted }) => {
    ...
    useEffect(() => {
        ...
        const startAudio = () => {
            ...
            const targetGain = isMobile ? 0.85 : 0.22;
            ...
            revFb.gain.value = 0.5;
            const revLpf = ctx.createBiquadFilter();
            revLpf.type = "lowpass";
            revLpf.frequency.value = 2800;
            const revWet = ctx.createGain();
            revWet.gain.value = 0.28;
            revDelay.connect(revLpf);
            revLpf.connect(revFb);
            ...
            bLpf.type = "lowpass";
            bLpf.frequency.value = 160;
            bLpf.Q.value = 1.8;
            const bG = ctx.createGain();
            bG.gain.value = 0.09;
            bassOsc.connect(bLpf);
            bLpf.connect(bG);
            bG.connect(master);
            bassOsc.start();
            s.srcs.push(bassOsc);
            ...
            [110, 130.81, 164.81, 196, 220].forEach((freq, i) => {
                ...
                lpf.connect(master);
                lpf.connect(revDelay);
            });

            // Breathing LFO
            const brLfo = ctx.createOscillator();
            brLfo.type = "sine";
            brLfo.frequency.value = 0.055;
            const brG = ctx.createGain();
            brG.gain.value = 0.03;
            brLfo.connect(brG);
            brG.connect(master.gain);
            brLfo.start();
            s.srcs.push(brLfo);
            ...
            nBpf.type = "bandpass";
            nBpf.frequency.value = 5800;
            nBpf.Q.value = 0.4;
            const nG = ctx.createGain();
            nG.gain.value = 0.006;
            noiseNode.connect(nBpf);
            nBpf.connect(nG);
            nG.connect(master);
            noiseNode.start();
            s.srcs.push(noiseNode);
            ...
            const fireNote = () => {
                ...
                o.detune.linearRampToValueAtTime(-8, now + 0.12);
                o.detune.linearRampToValueAtTime(0, now + 0.28);
                const g = ctx.createGain();
                g.gain.setValueAtTime(0, now);
                g.gain.linearRampToValueAtTime(0.042, now + 0.018);
            ...
            s.timers.push(
                setTimeout(() => {
                    fireNote();
                    s.timers.push(setInterval(fireNote, 320));
                }, 2500),
            );
            s.timers.push(
                setTimeout(() => {
                    firePulse();
                    s.timers.push(setInterval(firePulse, 1200));
                }, 1800),
            );
        };
